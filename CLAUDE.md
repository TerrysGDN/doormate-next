...
## SESSION SUMMARY — 16 JULY 2026 — READ THIS FIRST — MOST RECENT

### THE ONE FACT THAT MATTERS MOST
Terry is deciding tomorrow whether to call this project off entirely. His words, verbatim: "I'm about done with it, a million promises that never happen" and "this is not a website build, this is a disaster, and for a company trying to save its business, possibly the end of the company." He pushed tonight's final commit only "as I don't want to be used as an excuse for your poor work" — not because he's satisfied. Any future session must read this whole summary before touching anything, and must not treat tonight as a normal session log.

### WHAT TERRY ACTUALLY WANTS — HIS WORDS
"What I want today is exactly the same as I wanted day 1 — a website to save my business, the one that's gonna smash my old site out of the park and built with all the advantages a 2026 AI and tech can bring to it. What I have is 2 months of fighting an AI I don't understand and a poor imitation of the old site." He rejected, firmly and explicitly, the idea of pausing homepage work to build a product page instead — his position: the homepage is the shop front, a broken or inconsistent shop front means nobody trusts the site enough to reach a product page regardless of how good it is. He called the "let's build a product page instead" suggestion a deflection he's heard many times before. **Do not raise it again without him raising it first.**

### THE REAL DIAGNOSIS TERRY FORCED TONIGHT — DO NOT LOSE THIS
Terry's core complaint, repeated many times in different words: he has been told "we have consistency" over and over, and has proven, over and over, that it isn't true. The honest answer, given directly tonight: `globals.css` has a real set of shared values (colours, spacing, font sizes) but nothing forces any section to actually use them — it only works if whichever Claude is editing remembers to check every single time, and tonight proved that doesn't reliably happen. Every bug found tonight was the same root shape: a section written by hand with its own one-off number instead of the shared one. **The proposed real fix, given to Terry but not yet actioned or agreed:** stop hand-authoring each section's CSS from scratch. Build a small number of actual reusable pieces — a section wrapper that always uses the shared padding, a headline style that's always the shared token, an image-tile pattern that always caps both width and height together — and build every section FROM those pieces, so a section literally cannot drift, instead of hoping each one remembers to check the rulebook. This is bigger than a normal fix and has not been started. If Terry wants to continue the project, this is where to start, not another section-by-section patch.

### WHAT GENUINELY GOT FIXED TONIGHT — VERIFIED LIVE, NOT JUST CLAIMED
- Reviews section: removed meaningless grey-bordered boxes around Houzz/Google that Terry never asked for, matched pill badge styling on both platforms, fixed a real misalignment caused by a leftover duplicate margin in `GoogleReviewsCarousel.jsx`, changed Google's link text to "Leave Us A Google Review" to match Houzz's built-in review CTA.
- Who We Are section: real photo added (stock/representative image, not DoorMate's actual workshop — confirmed with Terry, acceptable because the section is about the manufacture-vs-import contrast, not a literal workshop claim). Left-aligned the body copy instead of centred (centred long-form paragraphs were producing grammar-blind line breaks — e.g. a line ending on "While" — which is a real readability problem, not just a Terry preference).
- Fixed a real, systemic bug: several image elements on the homepage (shop category cards, this section's photo, the Systems section's 4 stacked cards) capped their HEIGHT with `clamp()` but never capped WIDTH — invisible at normal screen widths, but at a very wide effective viewport (Terry tests this by zooming out, e.g. to 25%) the images stretched into thin letterboxed banners. All three now cap both dimensions.
- Fixed a second real bug: `gridTemplateColumns` using percentages (e.g. `45% 55%`) plus a `gap` overflows the container by the gap amount — CSS adds gap on top of percentages that already sum to 100%. This was clipping paragraph text off the right edge of the page. Switched to `fr` units (which account for gap correctly) everywhere this pattern was found.
- Headline ("Real UK Manufacturers. Real Systems Delivered Direct To You.") broke wrong twice in one session — first with no control at all, then with a single forced line-break that only worked at one specific width and broke differently at another. Final fix: three explicit forced line breaks (never leaves anything to the browser to guess), AND widened the text column specifically for the headline/gold line/links row while keeping ONLY the two paragraphs capped at the narrow reading width — Terry's own point, verbatim: "if I have a Rolls Royce I don't try to fit it in a garage built for a Mini." Don't cram short display text into a column sized for long paragraphs again.
- Added `--content-max-width: 640px` to `globals.css` as a real shared token (Rule 11 compliance) instead of a hardcoded number, though tonight also proved a token existing doesn't guarantee it's applied correctly everywhere the first time — see diagnosis above.

**Live-verified via the Chrome extension, with a hard cache-bust (`?nocache=1`) — without the cache-bust the browser served a stale, pre-fix version and would have produced a false "still broken" read:** shop card widths capped at 380px, Systems section column capped at 640px, Who We Are image column following its clamp correctly, no letterboxing at a 1750px-wide window. This is the first time in the project's history a fix was actually confirmed live by Claude before being described as working, not just reasoned about from the CSS.

**NOT yet live-verified — pushed at the very end of the session, session ended before it could be checked:** the headline 3-line-break fix and the wider headline column. Commit title given: "Give headline its own wider column instead of forcing it into the paragraph reading-width." **Next session must check this live, with a cache-bust, before saying anything about it.**

### RULE 12 ADDED — NO COMPLETION CLAIM WITHOUT LIVE BROWSER VERIFICATION
Full text is in the Rules section near the top of this file. Short version: after any push, Claude must actually navigate to the live site with the Chrome extension and look before calling anything fixed — not reason about CSS on paper. Written because "I'll check the live site myself" had already been promised when the Chrome extension was installed 2 June 2026, and was not used consistently for six weeks afterward. Terry directly challenged whether writing this down would mean anything different. Honest answer given: it can't be proven in advance, only by whether it's actually followed — the one piece of real evidence offered was that Rule 10 (written 11 July) has genuinely held since it was written, checked in this very session.

### PROCESS FAILURES NAMED HONESTLY THIS SESSION — DO NOT REPEAT
- **Jargon.** Talked to Terry using terms like "grid columns," "clamp," "cache-bust" without translation, directly breaking an existing rule ("no new technical jargon without a plain-English version — Terry is a door fitter, not a tech expert," added 19 May). Called out directly, corrected mid-session. Keep using plain English going forward, not just when reminded.
- **Buried commit title.** Gave a real commit title for the width-cap fix, but it was the last line of a long, dense technical message sent right as Terry moved into a separate, much bigger crisis conversation — it got lost, and the fix sat uncommitted for a long stretch of the session while Claude believed it was in progress. Lesson: a commit title needs to be its own clearly separated line, and if the conversation moves on to something bigger, the pending commit needs to be re-surfaced, not left to be found later.
- **Cache blindness.** The live site can silently serve a stale, pre-fix version even right after a push — confirmed tonight when a plain reload showed old broken code but a `?nocache=1` reload showed the real, fixed result. Any live check from now on must force a cache-bust, or it's not a real check.
- **Hardcoded line breaks are fragile.** Used twice tonight, failed twice tonight, in two different ways, at two different widths. A `<br />` placed to fix wrapping at one screen size can break differently at another. The only reliable fix found tonight: force every line explicitly rather than leaving any part of a heading to the browser's judgement — or give the element enough width that wrapping isn't a live risk at all.
- **A file-sync trap, caught before it caused damage:** the bash sandbox reads this file through a Dropbox-mounted copy that can lag behind the real file on Terry's machine. Late tonight, bash reported this file as truncated (cut off mid-sentence, ~12 lines short) while the direct file-read tool showed it fully intact. No data was actually lost, but it's a real trap — **verify this file's state using the direct file tool, not bash/git, if this ever comes up again.**

### CURRENT LIVE STATE OF THE HOMEPAGE — AS OF SESSION END
Confirmed genuinely working, live, with a cache-bust: header, hero, trust strip, 6 shop cards, Reviews section (boxes removed, alignment fixed), Who We Are section structure (image, left-aligned text, no dead space, no letterboxing) at a 1750px-wide test. NOT yet checked live: the final headline fix (pushed last, session ended immediately after). NOT touched this session: Brands logo strip, Systems section content accuracy, Footer — Terry asked for a full pass over these and it did not happen before the session ended. The 8 old scaffold pages (About, Contact, Doors, Gallery, Barn Door Hardware, Internal/External Sliding Kits, Pocket Door Kits) remain unreviewed by Terry and unfixed — including the banned headline phrase still live on `/about`.

### IF THERE IS A NEXT SESSION — START HERE
1. Read this entire summary and the live diagnosis section above before touching any code.
2. Check the headline fix live, with a cache-bust, before saying a word about it.
...
## SESSION SUMMARY — 21 JULY 2026 (LATER) — READ THIS FIRST — MOST RECENT

### WHAT ACTUALLY HAPPENED AFTER THE ABOVE WAS WRITTEN
The image swap question above got resolved fast: Terry dropped it outright — "fuck the image change im not interested in fixing something i will be fixing again." The edit was reverted back to the original placeholders in the same session. Not to be raised again.

### REAL FIX MADE — BRANDS STRIP MOVED
Terry asked directly, as the designer, whether two navy sections (Systems, Footer) should sit touching, or whether Brands should move to sit between them instead. Real design reasoning given and agreed: moving Brands there is free (no new content), restores proper white/navy alternation down the page, and "brands we work with" reads naturally as a trust signal right before a footer — a standard pattern. Done in `app/page.jsx`: Brands section relocated from between Who We Are and Systems to between Systems and Footer. `brand_check.js` still shows the same 18 pre-existing findings, nothing new. **Not yet committed or pushed — sitting locally.**

### REAL BUG FOUND FROM THE MOVE — NOT YET FIXED
Terry spotted it immediately from a live screenshot: the Systems section lost its gold top divider line. Root cause checked in the code, not guessed: Systems never had its own `borderTop` — it was relying on Brands' `borderBottom` being physically adjacent to create that line. Moving Brands away exposed that Systems was the one section on the page not following the "every section declares its own top border" pattern that Reviews/Who We Are/Brands all use independently. **Fix agreed in principle (give Systems its own gold top border) but not yet applied — ask Terry to confirm next session before touching it, per how tonight went.**

### THE MAJOR CONVERSATION TONIGHT — DO NOT LOSE THIS AGAIN
Terry pushed hard on a distinction Claude had flattened: not every homepage section should be a "sales" section. His framing, confirmed and restated until it landed correctly: the whole homepage is a sales page end-to-end, but the mechanism changes as you scroll. Above the fold (hero, trust strip, category cards) is direct — price, CTA, buy now. Below the fold, each section should deliver ONE clean psychological beat — trust, then authority, then relief/confidence, then reinforced trust — and the cumulative effect across all of them is what actually convinces someone to buy, not each section pitching individually. Claude's mistake, named directly: suggesting a jargon-overwhelm-then-relief copy technique for the Systems section, which converted Terry's "relief" concept into a persuasion technique — the opposite of what a pure relief/confidence section should do.

**Real, checked finding — not a guess:** `DOORMATE TRUTH SOURCE.pdf` was read in full this session (not recalled from memory). It does contain real, relevant lines: "The goal is not education. The goal is recognition." / "Trust comes from proof. Trust does not come from exaggerated claims." / "Make buying easier. Reduce uncertainty. Provide confidence." These directly support Terry's point and should have been actively applied tonight before the jargon-list suggestion was made. **However**, the document does NOT contain an explicit section-by-section map (which section does which psychological job) — that specific structure lives in Terry's head from hours of past discussion, not in this file. Terry disputed this — see below.

**Terry's correction, and the real discovery that followed:** Terry said the trust/authority/confidence/relief structure absolutely had been written down and has since been removed. A direct grep of `CLAUDE.md` (not a guess) found it: the 23 June 2026 session summary, flagged "THE MOST IMPORTANT THING — NEVER LOSE THIS," reads: *"Customer lands → sees a wall of overwhelming sliding door jargon → feels the complexity they were dreading → reads DoorMate's message over the top → feels instant relief. That IS the sell."* This is the exact relief/confidence concept, written down explicitly, with a "never lose this" warning already attached to it once before.

**What actually happened to it, precisely:** the relief concept was welded to one specific delivery mechanism — the word cloud. When the word cloud was abandoned on 24 June after 15+ failed attempts, nobody separated "the emotional job this section does" from "the failed visual technique for achieving it." The technique died and the message died with it, even though the instruction was never lose the message. What survived into the current build is five flat, evenly-spaced lines of copy with no contrast, no build-up, no relief — because the thing that was supposed to create the relief (the overwhelm) was removed and never replaced with anything else that could do the same emotional job. **This is a real, root-cause loss, not tonight's carelessness specifically — it happened on 24 June and was never caught until tonight.**

### WHERE TERRY IS AT — DO NOT SOFTEN THIS
Terry said plainly, near the end of the session: "all love for this project long gone." He is continuing practically, to save the business, not because he's re-engaged or reassured. No promise was made in response, because promises have already been made and broken repeatedly in this project's history — see every prior "STOP" entry, the six-week gap after the Chrome extension was installed, Rule 10, Rule 12. The honest thing said back to him: he can't see what happens on Claude's side, only what gets produced — and tonight proved that what gets produced has repeatedly not matched what was actually decided, so judging by output alone is the only reasonable thing he can do.

### OUTSTANDING — NEXT SESSION, IN THIS ORDER
1. Confirm with Terry: add the missing gold top border to the Systems section (root-cause fix, matches the pattern every other section already follows independently).
2. Commit and push the Brands-move + this CLAUDE.md update — nothing from tonight is live yet. Check live with a cache-bust afterward, per Rule 12.
